...
public class UserController {
    ...
    public void userAttendioninsert(Model model,HttpServletRequest request,HttpServletResponse response) throws IOException{
        ...
        Boolean aq=userService.userAttentioninsert(asd);
        if (aq){
            String result = gson.toJson(true);
            response.getWriter().append(result);
        }else {
    ...
    //取消对其他某个用户的关注
    @RequestMapping(value = "/userAttendiondelete.do",method = RequestMethod.POST)
    public void userAttendiondelete(Model model,HttpServletRequest request,HttpServletResponse response) throws IOException{
        request.setCharacterEncoding("utf-8");
        response.setCharacterEncoding("utf-8");
        InputStream is=request.getInputStream();
        BufferedReader reader=new BufferedReader(new InputStreamReader(is,"utf-8"));
        ...
        Gson gson=new Gson();
        int []asd=gson.fromJson(stringBuffer.toString(),int[].class);
        Boolean aq=userService.userAttendiondelete(asd);
        if (aq){
            String result = gson.toJson(true);
            response.getWriter().append(result);
        }else {
